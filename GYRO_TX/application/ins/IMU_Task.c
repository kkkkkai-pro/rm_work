...
#include <stdbool.h>
#include <stdio.h>
#include "usbd_cdc_if.h"   // USB 虚拟串口（Vofa+ 打印用）
...
/* ===== 磁力计使能开关 =====
 * 定义：启用 IST8310 九轴融合，Mahony 算法用地磁纠正 yaw 漂移（推荐，需硬件已接 IST8310）
 * 注释掉：退回六轴（陀螺仪+加速度计），yaw 会缓慢漂移，pitch/roll 不受影响
 * 若启用后 yaw 数据异常（乱跳/不变化），说明磁力计轴向或干扰有问题，改回注释即可 */
...
//   accel_fliter_1[2] = accel_fliter_2[2] = accel_fliter_3[2] = bmi088_real_data.accel[2];//z
    ...
    /* ===== 上电零偏自标定（新增）：这 ~3 秒内必须保持板子完全静止！=====
     * 采集 3000 次陀螺仪数据取平均作为零偏，替换原代码写死的他人板子标定值。 */
    {
        float gyro_sum[3] = {0.0f, 0.0f, 0.0f};
        for (uint16_t i = 0; i < 3000; i++)
        {
            BMI088_read(bmi088_real_data.gyro, bmi088_real_data.accel, &bmi088_real_data.temp);
            gyro_sum[0] += bmi088_real_data.gyro[0];
            gyro_sum[1] += bmi088_real_data.gyro[1];
            gyro_sum[2] += bmi088_real_data.gyro[2];
            osDelay(1);
        }
        bmi088_offset_data.gyro[0] = gyro_sum[0] / 3000.0f;
        bmi088_offset_data.gyro[1] = gyro_sum[1] / 3000.0f;
        bmi088_offset_data.gyro[2] = gyro_sum[2] / 3000.0f;
    }
...
//        IMU_task_add++;
		...
		IMU_angle[0] = 180.0f*INS_angle[0]/PI+180.0f;   //yaw_offset; //yaw
		IMU_angle[1] = 180.0f*INS_angle[1]/PI+180.0f; //pitch
		IMU_angle[2] = 180.0f*INS_angle[2]/PI+180.0f; //roll

        /* ===== Vofa+ 打印 Yaw,Pitch,Roll（度），FireWater 协议 ===== */
        {
            static char vofa_buf[64];
            int vofa_len = snprintf(vofa_buf, sizeof(vofa_buf), "%.3f,%.3f,%.3f\r\n",
                                    IMU_angle[0], IMU_angle[1], IMU_angle[2]);
            CDC_Transmit_FS((uint8_t *)vofa_buf, (uint16_t)vofa_len);
        }
